/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package ai.hanzo.cloud.model

import ai.hanzo.cloud.model.IamAddress
import ai.hanzo.cloud.model.IamCartItem
import ai.hanzo.cloud.model.IamConsentRecord
import ai.hanzo.cloud.model.IamFaceId
import ai.hanzo.cloud.model.IamManagedAccount
import ai.hanzo.cloud.model.IamMfaAccount
import ai.hanzo.cloud.model.IamMfaItem
import ai.hanzo.cloud.model.IamMfaProps
import ai.hanzo.cloud.model.IamPermission
import ai.hanzo.cloud.model.IamRole

import com.google.gson.annotations.SerializedName

/**
 * 
 *
 * @param accessKey API credentials. AccessSecret / AccessSecretHash / the OAuth tokens are bearer material. AccessSecretHash MUST persist (orm stores via JSON; a json:\"-\" field is never saved), so it carries a real json tag and the handler's redact() strips it (and AccessSecret + the token fields) before responding.
 * @param accessSecret 
 * @param accessSecretHash 
 * @param accessToken 
 * @param address 
 * @param addresses 
 * @param adfs 
 * @param affiliation 
 * @param alipay 
 * @param amazon 
 * @param apple 
 * @param applicationScopes 
 * @param auth0 
 * @param avatar 
 * @param avatarType 
 * @param azuread 
 * @param azureadb2c 
 * @param baidu 
 * @param balance Balance mirrors v1 for lossless migration but is authoritative in Commerce (billing.hanzo.ai), not here — do not write it from IAM.
 * @param balanceCredit 
 * @param balanceCurrency 
 * @param battlenet 
 * @param bilibili 
 * @param bio 
 * @param birthday 
 * @param bitbucket 
 * @param box 
 * @param cart 
 * @param cloudfoundry 
 * @param countryCode 
 * @param createdAt 
 * @param createdIp Sign-in provenance.
 * @param createdTime 
 * @param currency 
 * @param custom 
 * @param custom2 
 * @param custom3 
 * @param custom4 
 * @param custom5 
 * @param custom6 
 * @param custom7 
 * @param custom8 
 * @param custom9 
 * @param custom10 
 * @param dailymotion 
 * @param deezer 
 * @param deleted 
 * @param deletedTime 
 * @param digitalocean 
 * @param dingtalk 
 * @param discord 
 * @param displayName Profile.
 * @param douyin 
 * @param dropbox 
 * @param education 
 * @param email 
 * @param emailVerified 
 * @param eveonline 
 * @param externalId 
 * @param faceIds 
 * @param facebook 
 * @param firstName 
 * @param fitbit 
 * @param gender 
 * @param gitea 
 * @param gitee 
 * @param github Linked federated-identity subjects, one column per connector (v1 parity).
 * @param gitlab 
 * @param google 
 * @param groups 
 * @param hash 
 * @param heroku 
 * @param homepage 
 * @param iam 
 * @param id Id is the user's STABLE OPAQUE identifier — the value the OIDC `sub` claim carries. It is the v1 the legacy surface per-row UUID (e.g. \"e7d7fda0-4c53-4508-9d35-7ec892b7e5d7\"), migrated verbatim so a user's `sub` is byte-identical across the cutover: every live session, external reference, and the downstream money-path principal keyed on `sub` survive unchanged. A user minted natively in v2 is assigned a fresh UUID here on create, so the `sub` is ALWAYS a stable opaque id going forward — never the (Owner, Name) pair, which is mutable (a rename would otherwise silently reissue identity).  It is distinct from the embedded orm.Model STORAGE KEY — the value the datastore locks and looks a row up by — which is NOT (Owner, Name) for every row: a MIGRATED legacy row is stamped \"owner/name\" (SetId in the migrator), but a v2-native users.Create'd row is NOT — Create allocates rather than pinning a key, so its storage key is a store-assigned surrogate id (a decimal string like \"17847909129933610000001\"). (Owner, Name) is therefore the natural/QUERY key (unique, indexed), not necessarily the storage key: resolve a row for a locked write by its REAL key (store.GetUserByName(...).Key().Encode(), which stamps both shapes — see internal/oidc updateUser), never by assuming \"owner/name\". This Id is a first-class, indexed DOMAIN field; its json tag \"id\" dominates the promoted orm.Model `Id_` (also \"id\") by shallower depth, so the persisted record's \"id\" is this UUID — exactly the v1 shape. A row that carries no Id (a not-yet-assigned pre-cutover user) falls back to the (Owner, Name) subject at mint; every other path resolves `sub`→user by Id.
 * @param idCard 
 * @param idCardType 
 * @param influxcloud 
 * @param infoflow 
 * @param instagram 
 * @param intercom 
 * @param invitation 
 * @param invitationCode 
 * @param ipWhitelist 
 * @param isAdmin 
 * @param isDefaultAvatar State flags.
 * @param isDeleted 
 * @param isForbidden 
 * @param isOnline 
 * @param isVerified 
 * @param kakao 
 * @param karma 
 * @param kwai 
 * @param language 
 * @param lark 
 * @param lastChangePasswordTime 
 * @param lastName 
 * @param lastSigninIp 
 * @param lastSigninTime 
 * @param lastSigninWrongTime 
 * @param lastfm 
 * @param ldap 
 * @param line 
 * @param linkedin 
 * @param location 
 * @param mailru 
 * @param managedAccounts 
 * @param meetup 
 * @param mfaAccounts 
 * @param mfaEmailEnabled 
 * @param mfaItems 
 * @param mfaPhoneEnabled 
 * @param mfaPushEnabled 
 * @param mfaPushProvider 
 * @param mfaPushReceiver 
 * @param mfaRadiusEnabled 
 * @param mfaRadiusProvider 
 * @param mfaRadiusUsername 
 * @param mfaRememberDeadline 
 * @param mfaRememberDigest MfaRememberDigest is the digest of the token held by the ONE browser the deadline above applies to. Without it the deadline is account-wide and \"don't ask again on this browser\" switches the second factor off everywhere. It is a digest, never the token, so a database dump yields nothing presentable — and it carries a REAL json tag because orm persists via json.Marshal, so `json:\"-\"` would never be stored (the trap PasswordHash documents above); Mask() strips it from every response instead.
 * @param microsoftonline 
 * @param multiFactorAuths 
 * @param name 
 * @param naver 
 * @param needUpdatePassword 
 * @param nextcloud 
 * @param okta 
 * @param onedrive 
 * @param originalRefreshToken 
 * @param originalToken 
 * @param oura 
 * @param owner Identity / tenancy. (Owner, Name) is the natural key.
 * @param passwordHash Credential material. PasswordHash is a one-way bcrypt digest and is verify-only. It MUST be persisted (orm serializes the entity to its JSON data column, so a json:\"-\" field would never be stored — that silently broke login), so it carries a real json tag; the users API redact() strips it (and every other secret) from every response. PasswordType and PasswordSalt describe the digest scheme so rows hashed under the legacy argon2id scheme can still be verified and lazily re-hashed to bcrypt.
 * @param passwordSalt 
 * @param passwordType 
 * @param patreon 
 * @param paypal 
 * @param permanentAvatar 
 * @param permissions 
 * @param phone 
 * @param preHash 
 * @param preferredMfaType 
 * @param properties 
 * @param qq 
 * @param ranking 
 * @param realName 
 * @param recoveryCodes 
 * @param region 
 * @param registerSource 
 * @param registerType 
 * @param roles Authorization attachments. Roles and Permissions are computed on read from the authz store and carried here for API parity with v1.
 * @param salesforce 
 * @param score 
 * @param shopify 
 * @param signinWrongTimes 
 * @param signupApplication 
 * @param slack 
 * @param soundcloud 
 * @param spotify 
 * @param steam 
 * @param strava 
 * @param stripe 
 * @param tag 
 * @param telegram 
 * @param tiktok 
 * @param title 
 * @param totpSecret 
 * @param tumblr 
 * @param twitch 
 * @param twitter 
 * @param type 
 * @param typetalk 
 * @param uber 
 * @param updatedAt 
 * @param updatedTime 
 * @param verificationCode 
 * @param vk 
 * @param webauthnCredentials Multi-factor authentication. TotpSecret and RecoveryCodes are secret verify-only material — the handler strips them from every response. WebauthnCredentials is carried as raw JSON here for lossless migration; the typed passkey model is the sibling WebauthnCredential entity.
 * @param wechat 
 * @param wecom 
 * @param weibo 
 * @param wepay 
 * @param xero 
 * @param yahoo 
 * @param yammer 
 * @param yandex 
 * @param zoom 
 */


data class IamUser (

    /* API credentials. AccessSecret / AccessSecretHash / the OAuth tokens are bearer material. AccessSecretHash MUST persist (orm stores via JSON; a json:\"-\" field is never saved), so it carries a real json tag and the handler's redact() strips it (and AccessSecret + the token fields) before responding. */
    @SerializedName("accessKey")
    val accessKey: kotlin.String? = null,

    @SerializedName("accessSecret")
    val accessSecret: kotlin.String? = null,

    @SerializedName("accessSecretHash")
    val accessSecretHash: kotlin.String? = null,

    @SerializedName("accessToken")
    val accessToken: kotlin.String? = null,

    @SerializedName("address")
    val address: kotlin.collections.List<kotlin.String>? = null,

    @SerializedName("addresses")
    val addresses: kotlin.collections.List<IamAddress>? = null,

    @SerializedName("adfs")
    val adfs: kotlin.String? = null,

    @SerializedName("affiliation")
    val affiliation: kotlin.String? = null,

    @SerializedName("alipay")
    val alipay: kotlin.String? = null,

    @SerializedName("amazon")
    val amazon: kotlin.String? = null,

    @SerializedName("apple")
    val apple: kotlin.String? = null,

    @SerializedName("applicationScopes")
    val applicationScopes: kotlin.collections.List<IamConsentRecord>? = null,

    @SerializedName("auth0")
    val auth0: kotlin.String? = null,

    @SerializedName("avatar")
    val avatar: kotlin.String? = null,

    @SerializedName("avatarType")
    val avatarType: kotlin.String? = null,

    @SerializedName("azuread")
    val azuread: kotlin.String? = null,

    @SerializedName("azureadb2c")
    val azureadb2c: kotlin.String? = null,

    @SerializedName("baidu")
    val baidu: kotlin.String? = null,

    /* Balance mirrors v1 for lossless migration but is authoritative in Commerce (billing.hanzo.ai), not here — do not write it from IAM. */
    @SerializedName("balance")
    val balance: java.math.BigDecimal? = null,

    @SerializedName("balanceCredit")
    val balanceCredit: java.math.BigDecimal? = null,

    @SerializedName("balanceCurrency")
    val balanceCurrency: kotlin.String? = null,

    @SerializedName("battlenet")
    val battlenet: kotlin.String? = null,

    @SerializedName("bilibili")
    val bilibili: kotlin.String? = null,

    @SerializedName("bio")
    val bio: kotlin.String? = null,

    @SerializedName("birthday")
    val birthday: kotlin.String? = null,

    @SerializedName("bitbucket")
    val bitbucket: kotlin.String? = null,

    @SerializedName("box")
    val box: kotlin.String? = null,

    @SerializedName("cart")
    val cart: kotlin.collections.List<IamCartItem>? = null,

    @SerializedName("cloudfoundry")
    val cloudfoundry: kotlin.String? = null,

    @SerializedName("countryCode")
    val countryCode: kotlin.String? = null,

    @SerializedName("createdAt")
    val createdAt: java.time.OffsetDateTime? = null,

    /* Sign-in provenance. */
    @SerializedName("createdIp")
    val createdIp: kotlin.String? = null,

    @SerializedName("createdTime")
    val createdTime: kotlin.String? = null,

    @SerializedName("currency")
    val currency: kotlin.String? = null,

    @SerializedName("custom")
    val custom: kotlin.String? = null,

    @SerializedName("custom2")
    val custom2: kotlin.String? = null,

    @SerializedName("custom3")
    val custom3: kotlin.String? = null,

    @SerializedName("custom4")
    val custom4: kotlin.String? = null,

    @SerializedName("custom5")
    val custom5: kotlin.String? = null,

    @SerializedName("custom6")
    val custom6: kotlin.String? = null,

    @SerializedName("custom7")
    val custom7: kotlin.String? = null,

    @SerializedName("custom8")
    val custom8: kotlin.String? = null,

    @SerializedName("custom9")
    val custom9: kotlin.String? = null,

    @SerializedName("custom10")
    val custom10: kotlin.String? = null,

    @SerializedName("dailymotion")
    val dailymotion: kotlin.String? = null,

    @SerializedName("deezer")
    val deezer: kotlin.String? = null,

    @SerializedName("deleted")
    val deleted: kotlin.Boolean? = null,

    @SerializedName("deletedTime")
    val deletedTime: kotlin.String? = null,

    @SerializedName("digitalocean")
    val digitalocean: kotlin.String? = null,

    @SerializedName("dingtalk")
    val dingtalk: kotlin.String? = null,

    @SerializedName("discord")
    val discord: kotlin.String? = null,

    /* Profile. */
    @SerializedName("displayName")
    val displayName: kotlin.String? = null,

    @SerializedName("douyin")
    val douyin: kotlin.String? = null,

    @SerializedName("dropbox")
    val dropbox: kotlin.String? = null,

    @SerializedName("education")
    val education: kotlin.String? = null,

    @SerializedName("email")
    val email: kotlin.String? = null,

    @SerializedName("emailVerified")
    val emailVerified: kotlin.Boolean? = null,

    @SerializedName("eveonline")
    val eveonline: kotlin.String? = null,

    @SerializedName("externalId")
    val externalId: kotlin.String? = null,

    @SerializedName("faceIds")
    val faceIds: kotlin.collections.List<IamFaceId>? = null,

    @SerializedName("facebook")
    val facebook: kotlin.String? = null,

    @SerializedName("firstName")
    val firstName: kotlin.String? = null,

    @SerializedName("fitbit")
    val fitbit: kotlin.String? = null,

    @SerializedName("gender")
    val gender: kotlin.String? = null,

    @SerializedName("gitea")
    val gitea: kotlin.String? = null,

    @SerializedName("gitee")
    val gitee: kotlin.String? = null,

    /* Linked federated-identity subjects, one column per connector (v1 parity). */
    @SerializedName("github")
    val github: kotlin.String? = null,

    @SerializedName("gitlab")
    val gitlab: kotlin.String? = null,

    @SerializedName("google")
    val google: kotlin.String? = null,

    @SerializedName("groups")
    val groups: kotlin.collections.List<kotlin.String>? = null,

    @SerializedName("hash")
    val hash: kotlin.String? = null,

    @SerializedName("heroku")
    val heroku: kotlin.String? = null,

    @SerializedName("homepage")
    val homepage: kotlin.String? = null,

    @SerializedName("iam")
    val iam: kotlin.String? = null,

    /* Id is the user's STABLE OPAQUE identifier — the value the OIDC `sub` claim carries. It is the v1 the legacy surface per-row UUID (e.g. \"e7d7fda0-4c53-4508-9d35-7ec892b7e5d7\"), migrated verbatim so a user's `sub` is byte-identical across the cutover: every live session, external reference, and the downstream money-path principal keyed on `sub` survive unchanged. A user minted natively in v2 is assigned a fresh UUID here on create, so the `sub` is ALWAYS a stable opaque id going forward — never the (Owner, Name) pair, which is mutable (a rename would otherwise silently reissue identity).  It is distinct from the embedded orm.Model STORAGE KEY — the value the datastore locks and looks a row up by — which is NOT (Owner, Name) for every row: a MIGRATED legacy row is stamped \"owner/name\" (SetId in the migrator), but a v2-native users.Create'd row is NOT — Create allocates rather than pinning a key, so its storage key is a store-assigned surrogate id (a decimal string like \"17847909129933610000001\"). (Owner, Name) is therefore the natural/QUERY key (unique, indexed), not necessarily the storage key: resolve a row for a locked write by its REAL key (store.GetUserByName(...).Key().Encode(), which stamps both shapes — see internal/oidc updateUser), never by assuming \"owner/name\". This Id is a first-class, indexed DOMAIN field; its json tag \"id\" dominates the promoted orm.Model `Id_` (also \"id\") by shallower depth, so the persisted record's \"id\" is this UUID — exactly the v1 shape. A row that carries no Id (a not-yet-assigned pre-cutover user) falls back to the (Owner, Name) subject at mint; every other path resolves `sub`→user by Id. */
    @SerializedName("id")
    val id: kotlin.String? = null,

    @SerializedName("idCard")
    val idCard: kotlin.String? = null,

    @SerializedName("idCardType")
    val idCardType: kotlin.String? = null,

    @SerializedName("influxcloud")
    val influxcloud: kotlin.String? = null,

    @SerializedName("infoflow")
    val infoflow: kotlin.String? = null,

    @SerializedName("instagram")
    val instagram: kotlin.String? = null,

    @SerializedName("intercom")
    val intercom: kotlin.String? = null,

    @SerializedName("invitation")
    val invitation: kotlin.String? = null,

    @SerializedName("invitationCode")
    val invitationCode: kotlin.String? = null,

    @SerializedName("ipWhitelist")
    val ipWhitelist: kotlin.String? = null,

    @SerializedName("isAdmin")
    val isAdmin: kotlin.Boolean? = null,

    /* State flags. */
    @SerializedName("isDefaultAvatar")
    val isDefaultAvatar: kotlin.Boolean? = null,

    @SerializedName("isDeleted")
    val isDeleted: kotlin.Boolean? = null,

    @SerializedName("isForbidden")
    val isForbidden: kotlin.Boolean? = null,

    @SerializedName("isOnline")
    val isOnline: kotlin.Boolean? = null,

    @SerializedName("isVerified")
    val isVerified: kotlin.Boolean? = null,

    @SerializedName("kakao")
    val kakao: kotlin.String? = null,

    @SerializedName("karma")
    val karma: kotlin.Int? = null,

    @SerializedName("kwai")
    val kwai: kotlin.String? = null,

    @SerializedName("language")
    val language: kotlin.String? = null,

    @SerializedName("lark")
    val lark: kotlin.String? = null,

    @SerializedName("lastChangePasswordTime")
    val lastChangePasswordTime: kotlin.String? = null,

    @SerializedName("lastName")
    val lastName: kotlin.String? = null,

    @SerializedName("lastSigninIp")
    val lastSigninIp: kotlin.String? = null,

    @SerializedName("lastSigninTime")
    val lastSigninTime: kotlin.String? = null,

    @SerializedName("lastSigninWrongTime")
    val lastSigninWrongTime: kotlin.String? = null,

    @SerializedName("lastfm")
    val lastfm: kotlin.String? = null,

    @SerializedName("ldap")
    val ldap: kotlin.String? = null,

    @SerializedName("line")
    val line: kotlin.String? = null,

    @SerializedName("linkedin")
    val linkedin: kotlin.String? = null,

    @SerializedName("location")
    val location: kotlin.String? = null,

    @SerializedName("mailru")
    val mailru: kotlin.String? = null,

    @SerializedName("managedAccounts")
    val managedAccounts: kotlin.collections.List<IamManagedAccount>? = null,

    @SerializedName("meetup")
    val meetup: kotlin.String? = null,

    @SerializedName("mfaAccounts")
    val mfaAccounts: kotlin.collections.List<IamMfaAccount>? = null,

    @SerializedName("mfaEmailEnabled")
    val mfaEmailEnabled: kotlin.Boolean? = null,

    @SerializedName("mfaItems")
    val mfaItems: kotlin.collections.List<IamMfaItem>? = null,

    @SerializedName("mfaPhoneEnabled")
    val mfaPhoneEnabled: kotlin.Boolean? = null,

    @SerializedName("mfaPushEnabled")
    val mfaPushEnabled: kotlin.Boolean? = null,

    @SerializedName("mfaPushProvider")
    val mfaPushProvider: kotlin.String? = null,

    @SerializedName("mfaPushReceiver")
    val mfaPushReceiver: kotlin.String? = null,

    @SerializedName("mfaRadiusEnabled")
    val mfaRadiusEnabled: kotlin.Boolean? = null,

    @SerializedName("mfaRadiusProvider")
    val mfaRadiusProvider: kotlin.String? = null,

    @SerializedName("mfaRadiusUsername")
    val mfaRadiusUsername: kotlin.String? = null,

    @SerializedName("mfaRememberDeadline")
    val mfaRememberDeadline: kotlin.String? = null,

    /* MfaRememberDigest is the digest of the token held by the ONE browser the deadline above applies to. Without it the deadline is account-wide and \"don't ask again on this browser\" switches the second factor off everywhere. It is a digest, never the token, so a database dump yields nothing presentable — and it carries a REAL json tag because orm persists via json.Marshal, so `json:\"-\"` would never be stored (the trap PasswordHash documents above); Mask() strips it from every response instead. */
    @SerializedName("mfaRememberDigest")
    val mfaRememberDigest: kotlin.String? = null,

    @SerializedName("microsoftonline")
    val microsoftonline: kotlin.String? = null,

    @SerializedName("multiFactorAuths")
    val multiFactorAuths: kotlin.collections.List<IamMfaProps>? = null,

    @SerializedName("name")
    val name: kotlin.String? = null,

    @SerializedName("naver")
    val naver: kotlin.String? = null,

    @SerializedName("needUpdatePassword")
    val needUpdatePassword: kotlin.Boolean? = null,

    @SerializedName("nextcloud")
    val nextcloud: kotlin.String? = null,

    @SerializedName("okta")
    val okta: kotlin.String? = null,

    @SerializedName("onedrive")
    val onedrive: kotlin.String? = null,

    @SerializedName("originalRefreshToken")
    val originalRefreshToken: kotlin.String? = null,

    @SerializedName("originalToken")
    val originalToken: kotlin.String? = null,

    @SerializedName("oura")
    val oura: kotlin.String? = null,

    /* Identity / tenancy. (Owner, Name) is the natural key. */
    @SerializedName("owner")
    val owner: kotlin.String? = null,

    /* Credential material. PasswordHash is a one-way bcrypt digest and is verify-only. It MUST be persisted (orm serializes the entity to its JSON data column, so a json:\"-\" field would never be stored — that silently broke login), so it carries a real json tag; the users API redact() strips it (and every other secret) from every response. PasswordType and PasswordSalt describe the digest scheme so rows hashed under the legacy argon2id scheme can still be verified and lazily re-hashed to bcrypt. */
    @SerializedName("passwordHash")
    val passwordHash: kotlin.String? = null,

    @SerializedName("passwordSalt")
    val passwordSalt: kotlin.String? = null,

    @SerializedName("passwordType")
    val passwordType: kotlin.String? = null,

    @SerializedName("patreon")
    val patreon: kotlin.String? = null,

    @SerializedName("paypal")
    val paypal: kotlin.String? = null,

    @SerializedName("permanentAvatar")
    val permanentAvatar: kotlin.String? = null,

    @SerializedName("permissions")
    val permissions: kotlin.collections.List<IamPermission>? = null,

    @SerializedName("phone")
    val phone: kotlin.String? = null,

    @SerializedName("preHash")
    val preHash: kotlin.String? = null,

    @SerializedName("preferredMfaType")
    val preferredMfaType: kotlin.String? = null,

    @SerializedName("properties")
    val properties: kotlin.collections.Map<kotlin.String, kotlin.String>? = null,

    @SerializedName("qq")
    val qq: kotlin.String? = null,

    @SerializedName("ranking")
    val ranking: kotlin.Int? = null,

    @SerializedName("realName")
    val realName: kotlin.String? = null,

    @SerializedName("recoveryCodes")
    val recoveryCodes: kotlin.collections.List<kotlin.String>? = null,

    @SerializedName("region")
    val region: kotlin.String? = null,

    @SerializedName("registerSource")
    val registerSource: kotlin.String? = null,

    @SerializedName("registerType")
    val registerType: kotlin.String? = null,

    /* Authorization attachments. Roles and Permissions are computed on read from the authz store and carried here for API parity with v1. */
    @SerializedName("roles")
    val roles: kotlin.collections.List<IamRole>? = null,

    @SerializedName("salesforce")
    val salesforce: kotlin.String? = null,

    @SerializedName("score")
    val score: kotlin.Int? = null,

    @SerializedName("shopify")
    val shopify: kotlin.String? = null,

    @SerializedName("signinWrongTimes")
    val signinWrongTimes: kotlin.Int? = null,

    @SerializedName("signupApplication")
    val signupApplication: kotlin.String? = null,

    @SerializedName("slack")
    val slack: kotlin.String? = null,

    @SerializedName("soundcloud")
    val soundcloud: kotlin.String? = null,

    @SerializedName("spotify")
    val spotify: kotlin.String? = null,

    @SerializedName("steam")
    val steam: kotlin.String? = null,

    @SerializedName("strava")
    val strava: kotlin.String? = null,

    @SerializedName("stripe")
    val stripe: kotlin.String? = null,

    @SerializedName("tag")
    val tag: kotlin.String? = null,

    @SerializedName("telegram")
    val telegram: kotlin.String? = null,

    @SerializedName("tiktok")
    val tiktok: kotlin.String? = null,

    @SerializedName("title")
    val title: kotlin.String? = null,

    @SerializedName("totpSecret")
    val totpSecret: kotlin.String? = null,

    @SerializedName("tumblr")
    val tumblr: kotlin.String? = null,

    @SerializedName("twitch")
    val twitch: kotlin.String? = null,

    @SerializedName("twitter")
    val twitter: kotlin.String? = null,

    @SerializedName("type")
    val type: kotlin.String? = null,

    @SerializedName("typetalk")
    val typetalk: kotlin.String? = null,

    @SerializedName("uber")
    val uber: kotlin.String? = null,

    @SerializedName("updatedAt")
    val updatedAt: java.time.OffsetDateTime? = null,

    @SerializedName("updatedTime")
    val updatedTime: kotlin.String? = null,

    @SerializedName("verificationCode")
    val verificationCode: kotlin.String? = null,

    @SerializedName("vk")
    val vk: kotlin.String? = null,

    /* Multi-factor authentication. TotpSecret and RecoveryCodes are secret verify-only material — the handler strips them from every response. WebauthnCredentials is carried as raw JSON here for lossless migration; the typed passkey model is the sibling WebauthnCredential entity. */
    @SerializedName("webauthnCredentials")
    val webauthnCredentials: kotlin.collections.List<kotlin.Any>? = null,

    @SerializedName("wechat")
    val wechat: kotlin.String? = null,

    @SerializedName("wecom")
    val wecom: kotlin.String? = null,

    @SerializedName("weibo")
    val weibo: kotlin.String? = null,

    @SerializedName("wepay")
    val wepay: kotlin.String? = null,

    @SerializedName("xero")
    val xero: kotlin.String? = null,

    @SerializedName("yahoo")
    val yahoo: kotlin.String? = null,

    @SerializedName("yammer")
    val yammer: kotlin.String? = null,

    @SerializedName("yandex")
    val yandex: kotlin.String? = null,

    @SerializedName("zoom")
    val zoom: kotlin.String? = null

) {


}

