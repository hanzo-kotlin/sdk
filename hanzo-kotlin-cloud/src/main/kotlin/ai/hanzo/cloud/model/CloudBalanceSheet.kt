/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package ai.hanzo.cloud.model

import ai.hanzo.cloud.model.CloudBalanceLine

import com.google.gson.annotations.SerializedName

/**
 * 
 *
 * @param asOf 
 * @param assets 
 * @param balanced TotalAssets == TotalLiabilities + TotalEquity
 * @param equity 
 * @param liabilities 
 * @param totalAssets 
 * @param totalEquity 
 * @param totalLiabilities 
 */


data class CloudBalanceSheet (

    @SerializedName("asOf")
    val asOf: kotlin.String? = null,

    @SerializedName("assets")
    val assets: kotlin.collections.List<CloudBalanceLine>? = null,

    /* TotalAssets == TotalLiabilities + TotalEquity */
    @SerializedName("balanced")
    val balanced: kotlin.Boolean? = null,

    @SerializedName("equity")
    val equity: kotlin.collections.List<CloudBalanceLine>? = null,

    @SerializedName("liabilities")
    val liabilities: kotlin.collections.List<CloudBalanceLine>? = null,

    @SerializedName("totalAssets")
    val totalAssets: kotlin.Int? = null,

    @SerializedName("totalEquity")
    val totalEquity: kotlin.Int? = null,

    @SerializedName("totalLiabilities")
    val totalLiabilities: kotlin.Int? = null

) {


}

