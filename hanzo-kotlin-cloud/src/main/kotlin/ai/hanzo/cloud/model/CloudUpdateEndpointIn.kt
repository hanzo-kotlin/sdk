/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package ai.hanzo.cloud.model


import com.google.gson.annotations.SerializedName

/**
 * 
 *
 * @param description Description is a free-text label for the console. Optional, clipped to 1024 bytes.
 * @param events Events are NATS subject patterns to subscribe to. An empty or omitted list means EVERY event. Max 64 patterns, each max 256 bytes.
 * @param status Status is \"active\" or \"disabled\". Empty defaults to active.
 * @param url URL is the https:// address each matching event is POSTed to. Required, max 2048 bytes; http:// and every other scheme is refused.
 */


data class CloudUpdateEndpointIn (

    /* Description is a free-text label for the console. Optional, clipped to 1024 bytes. */
    @SerializedName("description")
    val description: kotlin.String? = null,

    /* Events are NATS subject patterns to subscribe to. An empty or omitted list means EVERY event. Max 64 patterns, each max 256 bytes. */
    @SerializedName("events")
    val events: kotlin.collections.List<kotlin.String>? = null,

    /* Status is \"active\" or \"disabled\". Empty defaults to active. */
    @SerializedName("status")
    val status: kotlin.String? = null,

    /* URL is the https:// address each matching event is POSTed to. Required, max 2048 bytes; http:// and every other scheme is refused. */
    @SerializedName("url")
    val url: kotlin.String? = null

) {


}

