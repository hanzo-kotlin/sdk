...
cloud client: every `/v1` route of the Hanzo API, 2468 operations, generated by
`./scripts/generate.sh` from the `openapi.yaml` hanzoai/cloud emits from its own
routers — `.spec-lock` names the release. Reach for it when the route is not in the
